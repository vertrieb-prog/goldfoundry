#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# GoldFoundry Signal Bot v8 - PURE ASCII (no unicode issues)
import sys, os
...
async def place_trade(http, aid, sig, bsym):
    ...
    if not sig["sl"]:
        try:
            ...
            if p:
                base = sig["entry"] or p
                # SL IMMER relativ zum aktuellen Preis (nicht Entry!)
                # BUY: SL unter aktuellem Preis, SELL: SL ueber aktuellem Preis
                sl_dist = 10 if ig else 0.003
                if ib:
                    sig["sl"] = round(p - sl_dist, 5)
                else:
                    sig["sl"] = round(p + sl_dist, 5)
                sig["entry"]=sig["entry"] or p
                print(f"   [SL] Auto: {sig['sl']} (from {base})")
    ...
    tps=list(sig["tps"])
    # IMMER 4 TPs - egal was im Signal steht
    base_price = p or sig["entry"] or 0  # IMMER aktuellen Preis nutzen
    if sig["sl"] and base_price:
        sd = abs(base_price - sig["sl"])
    else:
        sd = 10 if ig else 0.003
    while len(tps) < 4:
        m = [1.5, 2.5, 3.5, 5.0][len(tps)]
        tp_val = round(base_price + sd * m if ib else base_price - sd * m, 2)
        tps.append(tp_val)
    # Pending Order wenn Entry angegeben und weit vom Preis
    if sig["entry"] and p and abs(sig["entry"] - p) > (20 if ig else 0.002):
        if ib:
...
async def main():
    ...
    print("GoldFoundry Signal Bot v8")
    print("=" * 40)
