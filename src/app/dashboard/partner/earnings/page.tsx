...
export default function EarningsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#060503] text-white p-6 space-y-6">
      <h1 className="text-2xl font-bold text-[#d4a537]">Einnahmen & Auszahlungen</h1>

      {/* Transaction History */}
      <div className="bg-[#0a0a08] border border-[#1a1a15] rounded-xl p-6">
        <h2 className="text-lg font-semibold text-[#d4a537] mb-4">FP Transaktionen</h2>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {transactions.map((tx, i) => (
                <tr key={i} className="border-b border-[#1a1a15]/50">
                  <td className="py-2 px-3 text-gray-400">{tx.date}</td>
                  ...
                  <td className="py-2 px-3 text-right text-gray-400">{tx.balance.toLocaleString('de-DE')} FP</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {/* Payout Request */}
      <div className="bg-[#0a0a08] border border-[#1a1a15] rounded-xl p-6">
        <h2 className="text-lg font-semibold text-[#d4a537] mb-4">Auszahlung beantragen</h2>
        <p className="text-sm text-gray-400 mb-3">Mindestbetrag: 5.000 FP</p>
        <div className="flex gap-2 max-w-md">
          <input type="number" value={payoutAmount} onChange={(e) => setPayoutAmount(e.target.value)} placeholder="Betrag in FP" className="flex-1 bg-[#060503] border border-[#1a1a15] rounded-lg px-4 py-2 text-white placeholder-gray-600 focus:outline-none focus:border-[#d4a537]" />
          <button onClick={handlePayout} className="px-4 py-2 bg-[#d4a537] text-black rounded-lg font-semibold hover:bg-[#c4952f] text-sm">
      ...
      <div className="bg-[#0a0a08] border border-[#1a1a15] rounded-xl p-6">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-semibold text-[#d4a537]">Auszahlungsverlauf</h2>
          <button className="px-4 py-2 border border-[#d4a537] text-[#d4a537] rounded-lg text-sm font-semibold hover:bg-[#d4a537] hover:text-black transition-colors">
            CSV Export
          </button>
        </div>
        <div className="space-y-2">
          {payoutHistory.map((p, i) => (
            <div key={i} className="flex justify-between items-center px-4 py-3 bg-[#060503] rounded-lg border border-[#1a1a15]">
