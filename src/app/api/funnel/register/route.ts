...
export async function POST(request: Request) {
  try {
    ...
    // Email HTML
    const emailHtml = `<!DOCTYPE html><html><body style="margin:0;padding:0;background:#09090b;font-family:Inter,Arial,sans-serif;">
<div style="max-width:560px;margin:0 auto;padding:48px 24px;">
  <div style="text-align:center;margin-bottom:32px;">
    <div style="font-size:10px;letter-spacing:4px;color:#FAEF70;margin-bottom:8px;">GOLD FOUNDRY</div>
    <h1 style="color:#fafafa;font-size:24px;font-weight:800;margin:0 0 8px;">Willkommen${name ? `, ${name}` : ""}!</h1>
  ...
  <p style="color:#27272a;font-size:9px;text-align:center;">Gold Foundry &middot; goldfoundry.de &middot; Risikohinweis: Trading birgt Verlustrisiken.</p>
</div></body></html>`;

    // Push to GHL CRM + send email via GHL
    try {
      await pushLeadAndEmail(
        {
          firstName: name,
        ...
        "Bestätige deine Email — Gold Foundry",
        emailHtml
      );
      console.log("[FUNNEL] Lead + Email via GHL:", cleanEmail);
    } catch {
      // Fallback: send via Resend if GHL fails
      try {
        await sendEmail(cleanEmail, "Bestätige deine Email — Gold Foundry", emailHtml);
        console.log("[FUNNEL] Fallback email via Resend:", cleanEmail);
      } catch (e: any) {
        console.error("[FUNNEL] Both email methods failed:", e.message);
      }
    }
