...
async def main():
    ...
    async with aiohttp.ClientSession() as http:
        ...
        if not sessions or not sessions[0].get("session_string"):
            print("ERR Keine Telegram-Session!")
            return
    ...
    if not await client.is_user_authorized():
        print("ERR Session nicht autorisiert!")
        return
    ...
    async def handler(event):
        ...
        if not acc:
            print("   ERR Kein Account verknüpft!")
            return
        ...
        print(f"   {'HIT' if executed > 0 else 'ERR'} {executed} Order(s) in {ms}ms ({broker_sym})")
